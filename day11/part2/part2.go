	...
	for true {
		...
		for i := 0; i < rowCount; i++ {
			for j := 0; j < colCount; j++ {
				doField(&cavern, i, j, &flashedFields)
			}
		}
		...
		if len(flashedFields) == rowCount * colCount {
			break
		}
	}
...
func doField(cavern *[][]int, i, j int, flashedFields *[][]int) {
	(*cavern)[i][j]++
	if (*cavern)[i][j] > 9 {
		(*cavern)[i][j] = 0
		...
		*flashedFields = append(*flashedFields, []int{i, j})
		for m := -1; m < 2; m++ {
			for n := -1; n < 2; n++ {
				...
				doField(cavern, i2, j2, flashedFields)
			}
		}
	}
}
